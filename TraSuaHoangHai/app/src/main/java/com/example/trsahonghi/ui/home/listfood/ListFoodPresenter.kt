...
class ListFoodPresenter(
    ...
    override fun getListFood() {
        baseCallApi(foodRepository.getListFood(),
            onSuccess = { response ->
                if (response?.listFood.isNullOrEmpty()) {
                    _listFood.value = mutableListOf(
                        ...
                        BubbleTea(
                            R.drawable.ic_best_sell_1,
                            "Trà sữa hoàng gia kem phomai",
                            "30000"
                        ),
                    ...
                    _listFood.value = response?.listFood?.map { foodResponse ->
                        BubbleTea(
    ...
    override fun getBroadcastAction(listFood: MutableList<BubbleTea>): Pair<String, List<BubbleTea>> {
        ...
        val action = if (filteredList.isNotEmpty()) {
            ...
            Constants.Actions.NOTIFY_HIDE_CARD
        }
